...
public class CollectionPracticeProblems {
	...
	private String handleNonGradeStringError() {
		try {
			return returnStringIfIsAGrade(scanner.nextLine());
	...
	private String returnCountryAndPopulationIfFormatIsCorrect(String countryAndPopulation) throws WrongFormatException {
	...
	private String handleCountryAndPopulationFormatError() {
		try {
			...
			return returnCountryAndPopulationIfFormatIsCorrect(scanner.nextLine());
	...
	private Student createStudent(int studentNumber, boolean onlyNameAndGPA) {
		...
		if (onlyNameAndGPA) {
			...
			System.out.print("Enter student's concentration: ");
			concentration = scanner.nextLine();
			System.out.print("Enter student's grade: ");
			grade = handleParseIntError();
		}
		System.out.print("Enter student's GPA: ");
		gpa = handleParseDoubleError();
		return new Student(name, concentration, grade, gpa);
	}
	...
	private Location createLocation(int locationNumber) {
		...
		System.out.print("Enter city's latitude: ");
		latitude = handleParseDoubleError();
		System.out.print("Enter city's longitude: ");
		longitude = handleParseDoubleError();
		return new Location(city, latitude, longitude);
	}
	
	// countryAndCapital에 ","가 없으면 오류를 던지는 method
	private String returnCountryAndCapitalIfCorrectFormat(String countryAndCapital) throws WrongFormatException {
		if (!countryAndCapital.contains(",")) {
			throw new WrongFormatException("does not contain , between country name and capital name");
		}
		return countryAndCapital;
	}
	
	// 입력한 나라와 수도 문자열의 체재가 맞을때까지 실행되는 재귀 method. 
	private String[] handleCountryAndCapitalFormatError() {
		String countryAndCapital;
		String[] quit = { "quit" };
		try {
			System.out.print("Enter the names of the country and its capital (ex. USA, DC): ");
			countryAndCapital = scanner.nextLine();
			if (countryAndCapital.toLowerCase().contains("quit")) {
				return quit;
			}
			return returnCountryAndCapitalIfCorrectFormat(countryAndCapital).replaceAll(" ", "").split(",");
		} catch (WrongFormatException e) {
			System.out.println(e.getMessage());
			return handleCountryAndCapitalFormatError();
		}
	}
	...
	private void printAnswer(int problemNumber) {
		...
		case 2:
			...
			break;
		case 3:
			System.out.println("Enter a country and its population in this format (Korea, 1). To stop adding countries, enter \"quit\"");
	...
	String problem3() {
		...
		countryToLookUp = scanner.nextLine();
	...
	void problem4() {
		...
		do {
			...
			System.out.println("Current precipitation average: " + (precipitationLevels.stream().mapToInt(Integer::valueOf).sum() / precipitationLevels.size()));
	...
	void problem5() {
		...
		students.stream().forEach(
				student -> System.out.println(student.name + ", " + student.concentration + ", " + student.grade + ", " + student.gpa)
		);
		
		// 찾아보고 싶은 학생 이름을 studentToLookUp에 대입
		System.out.print("Enter the name of the student to look up: ");
		studentToLookUp = scanner.nextLine();
		// 찾아보고 싶은 학생 이름과 GPA 출력
		for (Student student : students) {
			if (student.name.equals(studentToLookUp)) {
				System.out.println("Student: " + studentToLookUp + " GPA: " + student.gpa);
			}
		}
	}

	/*
	 * 6. 도시 이름, 위도, 경도 정보를 가진 Location 클래스를 작성하고, 도시 이름을 '키'로 하는 HashMap<String, Location> 컬렉션을 만들고, 
	...
	void problem6() {
		...
		for (int i = 0; i < 4; i++) {
			...
			cities.put(location.city, location);
		}
		...
		while (shouldContinue) {
			...
			// location이 null이면 "City not in database" 출력
			if (location == null) {
				System.out.println("City not in database");
			// 아니면 도시 정보 출력
			} else {
				System.out.println("City: " + location.city + " Latitude: " + location.latitude + " Longitude: " + location.longitude);
			}
		}
	}
	...
	String problem7() {
		...
		for (int i = 0; i < 5; i++) {
			...
			students.put(student.name, student.gpa);
		}
		...
		for (Map.Entry<String, Double> registeredStudents : students.entrySet()) {
			...
			// registeredStudents의 gpa가 minimumGPA이상이면 studentsQualifiedForScholarshiptString에 이름을 더한다
			if (gpa >= minimumGPA) {
				studentsQualifiedForScholarshiptString += studentsQualifiedForScholarshiptString.equals("Students ") ? name : ", " + name;
			}
		}
		// 장학생들의 이름 반환
	...
	void problem8() {
		...
		while (true) {
			...
			while (!didEnterCorrectFormat) {
				...
				customerNameAndScore = scanner.nextLine(); 
	...
	void problem9() {
		...
		// 나라 이름과 수도를 저장할 문자열 배열 변수 ({나라 이름, 수도} 형식으로)
		String[] countryToAdd;
		...
		while (true) {
			...
			case 1:
				while (true) {
					// 입력할 나라와 수도를 countryToAdd에 대입
					countryToAdd = handleCountryAndCapitalFormatError();
					// countryToAdd[0]이 "quit"면 while 탈출
					if (countryToAdd[0].equals("quit")) {
						break;
					}
					// countryToAdd의 값들로 Country instance를 생성한후 country에 대입
					country = new Country(countryToAdd[0], countryToAdd[1]);
					// countries에 country가 이미 있으면 "(나라 이름) is already in the list" 출력
					if (countries.contains(country)) {
						System.out.println(country.countryName + " is already in the list");
					// 아니면 countries에 저장
					} else {
			...
			case 2:
				...
				for (int i = 0; i < countries.size(); i++) {
					...
					if (correctlyAnsweredCountries.get(country.countryName) == null) {
						System.out.print("What is the capital of " + country.countryName + "?: ");
						...
						} else if (capitalName.equals(country.capitalName)) {
							correctlyAnsweredCountries.put(country.countryName, true);
